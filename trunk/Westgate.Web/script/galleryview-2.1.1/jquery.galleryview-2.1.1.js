		...
		function buildFilmstrip() {
			...
			if(!opts.show_panels) { pointer.css('borderColor',transColor); }
				...
				switch(opts.filmstrip_position) {
					...
					case 'bottom': pointer.css({
										'top':(opts.panel_height-(pointer_height*2)+gallery_padding+filmstrip_margin)+5+'px',
				 						'left':((gallery_width-wrapper_width)/2)+(slide_method=='strip'?0:((f_frame_width+opts.frame_gap)*iterator))+((f_frame_width/2)-(pointer_width/2))+gallery_padding-28+'px',
										//'borderTopColor':transColor,
										//'borderRightColor':transColor,
										//'borderLeftColor':transColor
                                        'background':'url(../images/blue_bar.png) right top no-repeat'
									}); break;
			...
			if(filmstrip_orientation=='horizontal') {
				navNext.css({					 
					...
					'right':((gallery_width+(gallery_padding*2))/2)-(wrapper_width/2)-opts.frame_gap-22-25+'px'
				});
				navPrev.css({
					'top':(opts.filmstrip_position=='top'?Math.max(gallery_padding,filmstrip_margin):opts.panel_height+filmstrip_margin+gallery_padding)+((f_frame_height-22)/2)+'px',
				...
				navNext.css({					 
					'left':(opts.filmstrip_position=='left'?Math.max(gallery_padding,filmstrip_margin):opts.panel_width+filmstrip_margin+gallery_padding)+((f_frame_width-22)/2)+13-25+'px',
